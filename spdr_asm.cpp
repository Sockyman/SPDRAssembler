...
std::vector<char> assemble(string_list lines, const_map constants, bool print);
std::string line_as_string(int line_number, char value, std::string instruction, int operand);
...
int main(int argc, char *argv[])
{
	...
	std::vector<char> assembled = assemble(lines, constants, true);


	if (assembled.size() > 0)
	{
		std::ofstream output{"sample.o", std::ios::binary};
...
std::vector<char> assemble(string_list lines, const_map constants, bool print)
{
	...
	for (int i{0}; i < lines.size(); ++i)
	{
		...
		if (ins >= 0)
		{
			...
			int code = (ins << 4) | (operand & 0b1111);
			char byte_code = (char)code;
			assembled.push_back(byte_code);
			
			std::cout << line_as_string(assembled.size() - 1, byte_code, start, operand);
		}
		else if (start[0] == '#')
		{
			...
			std::cout << line_as_string(assembled.size() - 1, (char)byte_code, "#", byte_code);
		}
	}
